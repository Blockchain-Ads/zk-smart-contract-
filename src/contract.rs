//! BlockchainAds Campaign Selector contract
//!
//! Find decentralized relevant campaign for web3-cookie with zk-proof 
//!
//! This implementation works in following steps:
//!
//! 1. Initialization on the blockchain.
//! 2. Receival of multiple secret campaign, using the real zk protocol.
//! 3. Once enough campaigns have been received, the contract owner can start the ZK computation.
//! 4. The Zk computation process all the given campaign together related to web3-cookie params.
//! 5. Once the zk computation is complete, the contract will publicize the the selected campaign variable.
//! 6. Once the campaign variable is public, the contract will compute campaign hash and store it in
...
const BITLENGTH_OF_SECRET_CAMPAIGN_VARIABLES: u32 = 32;

/// Number of campaigns to wait for before starting computation. A value of 2 or below is useless.
const MIN_NUM_CAMPAIGNS: u32 = 3;
...
struct ContractState {
    ...
    /// Will contain the result when computation is complete
    selected_campaign_result: Option<u32>,
    /// Will contain the number of campaign after starting the computation
    num_campaigns: Option<u32>,
}
...
fn initialize(ctx: ContractContext, zk_state: ZkState<SecretVarMetadata>) -> ContractState {
    ContractState {
        ...
        selected_campaign_result: None,
        num_campaigns: None,
    }
}

/// Adds another campaign variable
///
/// The ZkInputDef encodes that the variable should have size [`BITLENGTH_OF_SECRET_CAMPAIGN_VARIABLES`].
...
fn add_campaign(
    ...
    assert!(
        ...
        "Each address is only allowed to send one campaign variable. Sender: {:?}",
        context.sender
    );
    let input_def = ZkInputDef {
        ...
        expected_bit_lengths: vec![BITLENGTH_OF_SECRET_CAMPAIGN_VARIABLES],
    };
    (state, vec![], input_def)
}
...
/// Allows the administrator to start the computation.
///
...
fn compute_selected_camaign(
    ...
    let num_campaigns = zk_state.secret_variables.len() as u32;
    assert!(num_campaigns >= MIN_NUM_CAMPAIGNS , "At least {} campaigns must have submitted and confirmed their inputs, before starting computation, but had only {}", MIN_NUM_CAMPAIGNS, num_campaigns);

    state.num_campaigns = Some(num_campaigns);
    (
        state,
        vec![],
...
fn open_sum_variable(
    ...
    let campaign = read_variable_u32_le(&zk_state, opened_variables.get(0));
//    let num_campaigns = state.num_campaigns.unwrap();
//    state.num_capmaigns = Some(num_campaigns);
    state.selected_campaign_result = campaign
